 ...
 */
@OperatorMetadata(alias = "adam.tile")
...
//          doExtract_simple();
    ...
    private void writeTileProduct(Product product, String tileName) {
        System.out.println("writing tile " + tileName);
        long sTime = System.currentTimeMillis();
        File dir = new File(bbdrDir, tileName);
        dir.mkdirs();
        File file = new File(dir, "subset_" + sourceProduct.getName() + "_ModisSDR_Geo.dim");
        WriteOp writeOp = new WriteOp(product, file, DimapProductConstants.DIMAP_FORMAT_NAME);
        writeOp.writeProduct(ProgressMonitor.NULL);
        long eTime = System.currentTimeMillis();
        int sec = (int) ((eTime-sTime) / 1000) % 60 ;
        int min = (int) (((eTime-sTime) / (1000*60)) % 60);
    ...
    static Product reproject(Product bbdrProduct, String tileName) {
        ...
        repro.setParameter("width", N2ModisConstants.MODIS_TILE_WIDTH);
        repro.setParameter("height", N2ModisConstants.MODIS_TILE_HEIGHT);
