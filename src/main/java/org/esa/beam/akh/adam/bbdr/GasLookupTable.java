...
import org.esa.beam.akh.adam.auxdata.Luts;
...
class GasLookupTable {
    ...
    void load(Product sourceProduct) throws IOException {
        if (sourceProduct != null && sensor == Sensor.VGT) {
            float ozoMeanValue = N2ModisUtils.getImageMeanValue(sourceProduct.getBand(N2ModisConstants.VGT_OZO_BAND_NAME).getGeophysicalImage());
...
//            setGasVal(0.242677f); // test!!
            ...
            setGasVal(N2ModisConstants.CWV_CONSTANT_VALUE);
        }
    ...
    private void loadCwvOzoLookupTableArray(Sensor sensor) throws IOException {
        ...
        try {
            ...
            if (this.sensor.equals(Sensor.VGT)) {
                int iOzo = N2ModisUtils.getIndexBefore(gas2val, ozoArray);
                ...
                int iCwv = N2ModisUtils.getIndexBefore(gas2val, cwvArray);
    ...
    float[] getTg(float amf, float gas) {
        int ind_amf = N2ModisUtils.getIndexBefore(amf, amfArray);
        float amf_p = (amf - amfArray[ind_amf]) / (amfArray[ind_amf + 1] - amfArray[ind_amf]);

        int ind_gas = N2ModisUtils.getIndexBefore(gas, gasArray);
